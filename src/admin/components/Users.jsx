...
//   const [addCategory, setAddCategory] = useState(false);
  ...
  const { users, loading } = useUsers(pages.page, pages.count, reload);
  
  const tabs = [
    { id: "all", label: "All", count: users.length || 0, data: users },
  ];

  return (
    <div>
      <p className="text-3xl text-gray-500 font-bold py-4">Users</p>
      <p className="text-gray-400 text-sm font-bold px-3">Active nalmart.com users</p>
      ...
      <UsersGrid users={tabs?.find(e => e.id === selectedTab).data} isLoading={ loading }/>
